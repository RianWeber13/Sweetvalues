...
<div class="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">
            {{ __('Editar Custo Indireto / Despesa') }}
        </h2>
    </div>

    <form method="POST" action="{{ route('overhead_costs.update', $overheadCost) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input id="name"
                ...
                type="text" name="name" value="{{ old('name', $overheadCost->name) }}" required autofocus />
            @error('name')
        ...
        <div class="mb-4">
            ...
            <select id="type" name="type"
                class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-pink-500">
                <option value="fixed" {{ old('type', $overheadCost->type) == 'fixed' ? 'selected' : '' }}>Valor Fixo
                    (R$)</option>
                <option value="percentage" {{ old('type', $overheadCost->type) == 'percentage' ? 'selected' : ''
                    }}>Porcentagem (%)</option>
            </select>
            @error('type')
            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
        ...
        <div class="mb-6">
            ...
            <input id="value"
                class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-pink-500"
                type="number" step="0.01" name="value" value="{{ old('value', $overheadCost->value) }}" required />
            @error('value')
            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
            @enderror
        </div>
